...
class ModernButton(QPushButton):
    """Custom button with modern styling and hover effects - Compact version"""
    def __init__(self, text="", icon=None, color="#4A90E2"):
        super().__init__(text)
        self.color = color
        self.setMinimumHeight(30)  # Reduced from 36
        self.setMaximumHeight(34)  # Added max height
...
class ModernCard(QGroupBox):
    """Card-style container with shadow effect - Compact version"""
...
class DataPreviewWidget(QWidget):
    ...
    def __init__(self):
        ...
        for label in [self.rows_label, self.cols_label, self.size_label, self.quality_label]:
            label.setStyleSheet("""
                QLabel {
                    font-size: 18px;
                    font-weight: bold;
        ...
        # Create descriptions
        desc_style = "QLabel { font-size: 12px; color: #666; font-weight: normal; }"
        rows_desc = QLabel("Rows")
        cols_desc = QLabel("Columns")
        size_desc = QLabel("Size")
        quality_desc = QLabel("Quality")
        
        for desc in [rows_desc, cols_desc, size_desc, quality_desc]:
            desc.setStyleSheet(desc_style)
        
        # Arrange in grid
        stats_layout.addWidget(rows_desc, 0, 0)
        stats_layout.addWidget(self.rows_label, 1, 0)
        stats_layout.addWidget(cols_desc, 0, 1)
        stats_layout.addWidget(self.cols_label, 1, 1)
        stats_layout.addWidget(size_desc, 0, 2)
        stats_layout.addWidget(self.size_label, 1, 2)
        stats_layout.addWidget(quality_desc, 0, 3)
        stats_layout.addWidget(self.quality_label, 1, 3)
        
        layout.addWidget(stats_card)
        
        # Preview table
        table_label = QLabel("📋 Data Sample (First 10 Rows)")
    ...
    def update_preview(self, dataframe):
        ...
        # UPDATED: Make columns manually expandable while setting intelligent defaults
        header = self.preview_table.horizontalHeader()
        
        # Set all columns to Interactive mode (allows manual resizing)
        ...
        for i, col_name in enumerate(display_df.columns):
            if col_name == 'Timestamp':
                # Timestamp needs more space
                self.preview_table.setColumnWidth(i, 160)
            elif col_name.endswith('_Status'):
                # Status columns are usually short
                self.preview_table.setColumnWidth(i, 80)
            elif display_df[col_name].dtype in ['float64', 'float32']:
                # Numeric columns need moderate space
...
class ZoomableChartView(QChartView):
    """Enhanced chart view with zoom capabilities"""
    ...
    def store_original_ranges(self):
        self.original_ranges = {axis: (axis.min(), axis.max()) for axis in self.chart().axes()}

    def reset_zoom(self):
        for axis in self.chart().axes():
            if axis in self.original_ranges:
                axis.setRange(*self.original_ranges[axis])

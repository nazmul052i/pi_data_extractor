    ...
    def run(self):
        try:
            ...
            for point in points:
                tag_info = {
                    'name': point.name,
                    'description': getattr(point, 'description', '').replace('\t', ' '),
...
class TagSearchDialog(QDialog):
    """Enhanced dialog for searching and selecting PI tags with multiple search capability"""
    ...
    def setup_ui(self):
        ...
        self.results_table.setHorizontalHeaderLabels(["Select", "Tag Name", "Description", "Units"])

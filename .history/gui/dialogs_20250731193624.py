...
class TagSearchDialog(QDialog):
    ...
    def add_selected_tags(self):
        ...
        for i in range(self.results_table.rowCount()):
            ...
            if checkbox and checkbox.isChecked():
                ...
                instrument = self.results_table.item(i, 4).text()  # Get instrument field
                ...
                if not any(tag['name'] == tag_name for tag in self.accumulated_tags):
                    tag_info = {
                        ...
                        'units': units,
                        'instrument': instrument  # Include instrument in tag data
                    }
    ...
    def show_accumulated_tags(self):
        ...
        dialog.resize(800, 400)  # Increased width for instrument column
        ...
        # List of tags with instrument column
        tag_list = QTableWidget()
        tag_list.setColumnCount(4)  # Increased from 3 to 4
        tag_list.setHorizontalHeaderLabels(["Tag Name", "Description", "Units", "Instrument"])
        tag_list.setRowCount(len(self.accumulated_tags))
        
        # Make columns resizable in the accumulated tags dialog too
        header = tag_list.horizontalHeader()
        for i in range(4):
            header.setSectionResizeMode(i, QHeaderView.ResizeMode.Interactive)
        
        # Set default widths for accumulated tags dialog
        tag_list.setColumnWidth(0, 180)  # Tag Name
        tag_list.setColumnWidth(1, 250)  # Description
        tag_list.setColumnWidth(2, 80)   # Units
        tag_list.setColumnWidth(3, 150)  # Instrument
        ...
        for i, tag in enumerate(self.accumulated_tags):
            ...
            tag_list.setItem(i, 3, QTableWidgetItem(tag.get('instrument', '')))

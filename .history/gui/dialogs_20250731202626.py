...
class ProgressDialog(QDialog):
    ...
    def update_progress(self, value, status="", detail=""):
        ...
        if detail:
            self.detail_label.setText(detail) __init__(self, server, search_pattern, max_results):
        super().__init__()
        self.server = server
    ...
    def get_instrument_info(self, point):
        ...
        # Method 9: Extract from tag name (common pattern: INSTRUMENT_TAG)
        tag_name = point.name
        if '_' in tag_name:
        ...
        # Method 10: Extract from tag name (pattern: UNIT.AREA.INSTRUMENT.TAG)
        if '.' in tag_name:

...
class EnhancedPIDataExtractorGUI(QWidget):
    ...
    def export_data(self):
        """Export data in selected format (CSV, TXT, or IQ)"""
        ...
        try:
            ...
            elif format_selected == ".iq":  # ADD THIS NEW CONDITION
                exporter.export_iq(file_path)
                self.log_output.append(f"✅ Data exported to IQ format (tab-delimited): {file_path}")
...
def browse_export_path(self):
    ...
    elif selected_format == ".iq":  # ADD THIS NEW CONDITION
        file_filter = "IQ Files (*.iq);;Text Files (*.txt);;All Files (*)"
    else:  # .txt

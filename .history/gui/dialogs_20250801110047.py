...
class TagSearchWorker(QThread):
    ...
    def get_instrument_info(self, point):
        """Extract instrument information using multiple methods - FIXED for 'Instrument tag' attribute"""
        
        # Method 1: Try the "Instrument tag" attribute (the correct one!)
        instrument = self.safe_get_attribute(point, 'Instrument tag', '')
        if instrument:
            return instrument
        
        # Method 2: Try other variations of instrument tag attributes (case variations)
        instrument_variations = [
            'Instrument tag',     # lowercase
            'Instrument Tag',     # Title Case
            'INSTRUMENT TAG',     # UPPERCASE
            'InstrumentTag',      # camelCase
            'instrumentTag',      # camelCase variation
        ]
        
        for attr_name in instrument_variations:
            instrument = self.safe_get_attribute(point, attr_name, '')
            if instrument:
                return instrument
        
        # Method 3: Try other common instrument-related attributes (fallbacks)
        fallback_attrs = [
            'instrument',         # original attempt
            'instrumenttag',      # original attempt
            'equipment',
            'asset',
            'source',
            'pointsource'
        ...
        # Method 4: Try location attributes
        for i in range(1, 5):
            instrument = self.safe_get_attribute(point, f'location{i}', '')
            if instrument:
                return instrument
        
        # Method 5: Try area/unit/plant attributes
        for attr_name in ['area', 'unit', 'plant', 'system', 'subsystem']:
            instrument = self.safe_get_attribute(point, attr_name, '')
            if instrument:
                return instrument
        
        # Method 6: Extract from tag name (common pattern: INSTRUMENT_TAG)
        tag_name = point.name
        if '_' in tag_name:
        ...
        # Method 7: Extract from tag name (pattern: UNIT.AREA.INSTRUMENT.TAG)
        if '.' in tag_name:
            parts = tag_name.split('.')
            if len(parts) >= 3:
                return parts[2]  # Third part is often the instrument
        
        # Method 8: Extract from tag name with dash separator
        if '-' in tag_name:
            parts = tag_name.split('-')
